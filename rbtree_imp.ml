(* 
   ...
   Copyright (C) 2009,2010,2011  Didier Cassirame
...
sig
  ...
  val create     : unit -> t
  val is_empty   : t -> bool
  val fold_left  : ('b -> cell_t -> 'b) -> 'b -> t -> 'b
  val fold_right : (cell_t -> 'b -> 'b) ->  t -> 'b -> 'b
  val iter       : (cell_t -> unit) -> t -> unit
  val add        : t -> cell_t -> unit
...
struct
  ...
  (* check whether the given node/tree is empty *)
  ...
  let fold_right f tree acc =
    let rec fold_right_ f node acc =
      match node.v with
	  EmptyContent -> acc
	| Content v    ->
	  fold_right_ f  node.l (f (fold_right_ f node.r acc) v)
	    
    in fold_right_ f tree.root acc
  ...
  let add tree v =
    ...
    *)
    ...
    let rec add_r parent node v =
      match node.v with 
	  EmptyContent ->
	    parent.r <- (create_node v); 2 (* ensure that we're going to check at least parent and gparent *)
	| Content nv   -> 
    ...
    and add_l parent node v =
      match node.v with 
	  EmptyContent ->
	    parent.l <- (create_node v); 2 (* ensure that we're going to check at least parent and gparent *)
	| Content nv   -> 
	  match compare v nv with
	      Below -> check_bal_l node (add_l node node.l v)
	    | Above -> check_bal_r node (add_r node node.r v)
